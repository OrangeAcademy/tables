...
function useAutobook() {
  ...
  */
  const automaticallyBookMeeting = useCallback(
    async (meetingDuration) => {
      await dispatch(postEvents(newReservationTemplate(meetingDuration)));
      await handlePostAndUpdate();
      return;
    },
    [dispatch, handlePostAndUpdate]
  );
